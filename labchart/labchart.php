<?php	
    ...
    /* lachart.php: Script principal de la aplicaci�n. Se lo consulta desde el frontend cuando el usuario selecciona un piso. Devuelve un JSON 
     * con la data preparada para su visualizaci�n; o frena el proceso y devuelve string("usar cache") en caso de que el cache sea muy reciente.
     * Entrada: $_GET['piso'] existe en array(1, 3, 5, 6, 7, 8, 9)
     * Funciones: Se encarga de recopilar los datos, conexi�n con la base de datos del laboratorio, darles la estructura, preprocesarlos, analizarlos,
     * y hacerlos disponibles al frontend. 
     * Devuelve: echo array_JSON
     */
    
    ### CONFIGURACI�N Y PAR�METROS INICIALES 
    ...
    $piso = $_GET['piso']; // �nica variable de entrada: El piso al cual se hace la consulta.   
    ...
    ### SISTEMA DE CACHE:
    /* 
    Mejora el rendimiento de la aplicaci�n. El JSON final que devuelve labchart.php queda almacenado como archivo /cache/cache_piso_X.json
    *  al final del proceso. Si existe un cache m�s reciente que $duracion_del_cache (por defecto: 2 min), reutiliza ese JSON almacenado 
    * transmitiendo al frontend "usar cache" y detiene el script.
    * Si es m�s antiguo que $duracion_del_cache, el cache ser� lo que se muestre inicialmente hasta que labchart.php termine de ejecutarse
    * y envie al frontend los datos actualizados.   */ 
    ...
    ### INTERFAZ V�A WEB-SERVICE CON LA BASE DE DATOS (Laboratorio y HCA)
    function pacientes_por_piso($piso) {	
        /* Consulta al web-service funci�n 'pacientes', organiza los datos recibidos en un array de estructura (HC, Nombre, Cama)
    ...
    function ordenes_de_paciente($HC) {	
        /* Consulta al web-service funci�n ordenestot, junta todas las ordenes de un determinado paciente (identificado por N� de HC) .
    ...
    function procesar_estudio($orden, $timestamp) {
        ...
        foreach ($estudio_raw as $estudio) {	
            $codigo = $estudio['codanalisi']; // 3 o 4 letras en may�scula que identifican un estudio. Ej: HTO = Hematocrito.
            ...
            // Comienza a crear el array que devuelve la funci�n
            ...
            'color' => "black", // Color con el que se mostrar�. Las alertas pueden modificar este par�metro m�s adelante
            'info' => ""  // Snippet que se muestra al poner el mouse sobre el valor. Las alertas pueden agregar info ac�.
            );
        }
        
        // Postprocesamiento de los datos.
        # Ordena los resultados seg�n el orden predefinido en agrupar_estudios_array: primero el orden de los grupos, luego orden de estudios.
    ...
    ### MODULO DE AN�LISIS DE LOS RESULTADOS Y GENERACI�N DE ALERTAS
    function analisis_de_alertas($estudios_de_hoy, $todos_los_estudios, $piso) {
         ...
         * una descripci�n del alerta en el campo "info", la cual se mostrar� en un snippet por el frontend. Adem�s, cambia el valor "color" en 
         * ese mismo array, lo cual se ver� reflejado desde el frontend con un cambio en el color del texto de dicho resultado.
         * 
         * Entrada: $estudios_de_hoy: Se limita a los resultados de las �ltimas 24hs, que son los que se analizan y se muestran en el frontend.
         *          $todos_los_estudios: Se utiliza solo con fines de an�lisis, para comparar los $estudios_de_hoy  con otros resultados previos. 
         ...
         * Funcionamiento: Itera cada uno de las �rdenes de estudios, luego itera sobre cada estudio individual (Hematocrito, hemoglobina, etc).
         ...
         * pueden ser comparaciones relativas a valores previos, en dicho caso tambi�n itera sobre los estudios previos para compararlos.
        ...
        foreach($estudios_de_hoy as $key_estudio => $estudio_analizado) {
            foreach(array_slice($estudio_analizado, 5, -2) as $key_grupos => $grupo_de_estudios) {
                foreach($grupo_de_estudios as $key_codigo => $array_resultado) {
                    ...
                    if (!is_numeric($resultado_de_hoy)) { # Esto podr�a cambiarse m�s adelante, pero en principio solo analiza resultados num�ricos.
                            continue; 
                    }
                    ...
                    if ($key_codigo == "HTO") { 
                        ...
                        # Analisis comparativo con resultados previos. Busca ca�das bruscas del hematocrito en las �ltimas 96hs.
                    ...
                    if ($key_codigo == "CRE") { 
                        ...
                        #Comparaci�n con resultados de dias previos
    ...
    function textificar_array($estudios) {
                    /* Resume el resultado de un laboratorio entero en dos strings, y las agrega al array de estudios.
                            ...
                            - $textificado_largo: Modo expresivo, completo, con unidades, que cumple con formalidades m�dico-legales
        ...
        foreach ($estudios as $key => $solicitud) {
            ...
            foreach(array_slice($solicitud, 5) as $key_grupos => $grupo_de_estudios) {
                                    ...
                                    // Formateo de estado �cido base / gasometria
                ...
                // NOTA: En textificado_corto, s�lo se muestran los estudios especificamente incluidos en el array $abreviar_nombres_estudios
...
/*  function agrupar_por_pacientes($array_original, $pacientes) {
        ...
        # Quiz�s utilice esto en algun momento para emparchar la estructura de datos. El objetivo es  agrupar los estudios por paciente. 
