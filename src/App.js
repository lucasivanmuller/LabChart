...
async function checkLogin() {
  if (no_login) {
    console.log("Modo no login")
    return false
  }
  let response = await fetch(`../check_login.php`, {
...
function App() {
  ...
  useEffect(() => {
    fetch(
      debugging
        ? `http://localhost/labchart/get_cache.php?piso=${activeFloor}`
        : `../labchart/get_cache.php?piso=${activeFloor}`,
  ...
  useEffect(() => {
    fetch(
      debugging
        ? `http://localhost/labchart/labchart.php?piso=${activeFloor}`
        : `../labchart/labchart.php?piso=${activeFloor}`,
      {
        method: "get",
        signal: signal,
        credentials: "same-origin",
  ...
  return (
    <Layout>
        ...
        <div style={{ float: "right" }}>
          ...
          >
            <input
              ...
              id="submit2"
              value="Cerrar sesion"
              style={{ float: "right", lineHeight: "25px" }}
            />
          </form>
        </div>
      </Header>
                            ...
                            return (
                              <Col>
                                <Space direction="vertical">
                                  ...
                                  <CopyToClipboard
                                    text={`${rest["text_largo"]}`}
                                  >
                                    <button>Copiar completo</button>
                                  </CopyToClipboard>
                                </Space>
                              </Col>
                            )
                            ...
                            <Col>
                              <Space direction="vertical">
                                ...
                                {Object.keys(rest[title]).map((prop) => (
                                  <Tooltip
                                    label={`${rest[title][prop].info}`}
                                    style={
                                      rest[title][prop].info === ""
      ...
      <Footer>
        ...
        <div>
          <i>
            Tip: Si mantenés el mouse sobre un valor resaltado, podés obtener
        ...
        <div style={{ marginTop: "20px" }}>
          <a
            ...
            href="https://www.paho.org/es/argentina/calidad-salud/reconocimiento-buenas-practicas-seguridad-pacientes#cat2"
            target="_blank"
            rel="noopener noreferrer"
          >
            {" "}
            Reconocido como buena práctica de seguridad del paciente por:{" "}
            {"    "}
            <img
              src="https://www.paho.org/themes/paho/images/logo-es.png"
              alt="Logo OPS"
